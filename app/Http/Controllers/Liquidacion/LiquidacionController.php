...
class LiquidacionController extends ApiController
{
  ...
  public function calcularBruto(Request $request)
  {
    ...
    $resp_bruto = DB::select(
      DB::raw("SELECT mat_matricula , SUM(os_precio_total) AS total
  ...
  public function liquidar(Request $request)
  {
    ...
    $this->liquidacionDetalle = $this->obtenerLiquidacionDetalle(
      $id_liquidacion_generada
    ); // DEBE VENIR DEL REQUEST
    ...
    foreach ($this->liquidacionDetalle as $index => $_liquidacionDetalle) {
      if ($_liquidacionDetalle->os_liq_neto == 0) {
        ...
        echo "matricula " .
          $_liquidacionDetalle->mat_matricula .
          "   " .
          $_liquidacionDetalle->os_liq_neto;
        ...
        if ($_liquidacionDetalle->mat_matricula) {
          ...
          $this->TOTAL_PERCEPCIONES = $this->calcularPercepciones(
            $_liquidacionDetalle
          );
          if ($this->tieneSaldo($this->saldo, $this->TOTAL_PERCEPCIONES)) {
            $this->saldo = $this->saldo - $this->TOTAL_PERCEPCIONES;
          }
          // OBTENGO LA DEUDA Y PRUEBO DESCONTAR
          $this->deuda = $this->obtenerDeudaMatricula(
            $_liquidacionDetalle->mat_matricula
          );

          $this->pago_historico_ids = $this->calcularDeudaMatricula();

          if ($this->pago_historico_ids !== "") {
            // SI NO VIENE VACIO ACTUALIZO LOS VALORES DE MATRICULA
            $this->actualizarDeudaLiquidacion(
              $this->pago_historico_ids,
              $_liquidacionDetalle->id_liquidacion_detalle,
              $_liquidacionDetalle->mat_matricula
            );
          }
        }
        // ACTUALIZO TODAS LAS DEDUCCIONES
        $this->actualizarLiquidacionDetalleConceptos(
          $_liquidacionDetalle->id_liquidacion_detalle
        );
    ...
    if ($this->INTERESES_pago_historico_id !== "") {
      // SI NO VIENE VACIO ACTUALIZO LOS VALORES DE MATRICULA
      $this->actualizarDeudaLiquidacionInteres(
  ...
  {
    $res = DB::select(
      DB::raw("SELECT id, mat_matricula_psicologo, CONCAT(mat_apellido, ' ', mat_nombre) AS mat_nombre_apellido
  ...
  {
    $res = DB::select(
      DB::raw("SELECT id_liquidacion_detalle, os_liq_detalle, os_liq_monto_porcentaje, os_liq_tipo FROM os_liq_percepcion WHERE 1
    ")
    );
  ...
  {
    $res = DB::select(
      DB::raw("SELECT id_liquidacion_detalle, mat_matricula,
  ...
  public function obtenerLiquidacionDetalleSeleccionadas(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw(
        "SELECT id_liquidacion_detalle, mat_matricula,
    os_liq_bruto, os_ing_brutos, os_lote_hogar, os_gasto_admin, os_imp_cheque,
    os_descuentos, os_desc_matricula, os_desc_fondo_sol,
    os_otros_ing_eg, os_liq_neto, num_comprobante,
    os_num_ing_bruto, id_liquidacion_generada
  ...
  private function obtenerDeudaMatricula($mat_matricula)
  {
    ...
    (SELECT id_pago_historico, mat_matricula, mat_fecha_pago, mat_fecha_vencimiento, mat_pago_historico.mat_monto, mat_monto_cobrado, mat_num_cuota, mat_pago_historico.mat_descripcion, mat_id_plan, mat_pago_historico.id_concepto, mat_estado, mat_concepto.mat_concepto, mat_interes FROM mat_pago_historico, mat_concepto WHERE mat_pago_historico.id_concepto = 11  AND mat_pago_historico.mat_estado = 'A' AND mat_pago_historico.mat_matricula =" .
          $mat_matricula .
          " AND mat_concepto.id_concepto = mat_pago_historico.id_concepto AND ((`mat_fecha_vencimiento`) <=curdate())    ORDER BY mat_pago_historico.mat_fecha_vencimiento ASC  LIMIT 1 )
    UNION
    (SELECT id_pago_historico, mat_matricula, mat_fecha_pago, mat_fecha_vencimiento, mat_pago_historico.mat_monto, mat_monto_cobrado, mat_num_cuota, mat_pago_historico.mat_descripcion, mat_id_plan, mat_pago_historico.id_concepto, mat_estado, mat_concepto.mat_concepto, mat_interes FROM mat_pago_historico, mat_concepto WHERE mat_pago_historico.id_concepto NOT IN (1,2,10,11)  AND mat_pago_historico.mat_estado = 'A' AND mat_pago_historico.mat_matricula =" .
          $mat_matricula .
          " AND mat_concepto.id_concepto = mat_pago_historico.id_concepto AND ((`mat_fecha_vencimiento`) <=curdate())    ORDER BY mat_pago_historico.mat_fecha_vencimiento ASC )
    "
      )
    );
  ...
  {
    $res = DB::select(
      DB::raw("SELECT id_concepto, mat_concepto, mat_monto, mat_interes, mat_descripcion FROM mat_concepto WHERE mat_monto !=0
    ")
    );
    return $res;
  }
  ...
  private function calcularPercepciones($_liquidacionDetalle)
  {
    ...
    if ($_saldo_restante >= $this->TOTAL_os_ing_brutos_limite) {
      ...
      $_saldo_restante = $_saldo_restante - $this->TOTAL_os_ing_brutos; // ($_saldo_restante * $this->os_ing_brutos);
      if ($this->tieneSaldo($_saldo_restante, $this->os_lote_hogar)) {
        $this->TOTAL_os_lote_hogar =
          $this->TOTAL_os_ing_brutos * $this->os_lote_hogar;
        $_saldo_restante =
          $_saldo_restante - $_saldo_restante * $this->os_lote_hogar; // VERIFICAR .... NO PARECE CORRECTO
      }
    }
    ...
    if (
      $this->tieneSaldo(
        $_saldo_restante,
        $_saldo_restante * $this->os_gasto_admin
      )
    ) {
      $this->TOTAL_os_gasto_admin =
        $_liquidacionDetalle->os_liq_bruto * $this->os_gasto_admin;
      echo "ga" .
        round($this->TOTAL_os_gasto_admin, 2, PHP_ROUND_HALF_UP) .
        " ";
      $_saldo_restante = $_saldo_restante - $this->TOTAL_os_gasto_admin;
    }

    if (
      $this->tieneSaldo(
        $_saldo_restante,
        $_saldo_restante * $this->os_imp_cheque
      )
    ) {
      $this->TOTAL_os_imp_cheque =
        $_liquidacionDetalle->os_liq_bruto * $this->os_imp_cheque;
      $_saldo_restante = $_saldo_restante - $this->TOTAL_os_imp_cheque;
    }
    ...
    $_TOTAL =
      round($this->TOTAL_os_ing_brutos, 2, PHP_ROUND_HALF_UP) +
      round($this->TOTAL_os_lote_hogar, 2, PHP_ROUND_HALF_UP) +
      round($this->TOTAL_os_gasto_admin, 2, PHP_ROUND_HALF_UP) +
      round($this->TOTAL_os_imp_cheque, 2, PHP_ROUND_HALF_UP);
  ...
  private function calcularDeudaMatricula()
  {
    ...
    foreach ($this->deuda as $index => $_deuda) {
      if ($this->tieneSaldo($_saldo_restante, $_deuda->mat_monto)) {
        if (
          $_deuda->id_concepto === 1 ||
          $_deuda->id_concepto === 10 ||
          $_deuda->id_concepto === 16
        ) {
          //CALCULO LOS INTERESES SI ESTA VENCIDA
          if (
            $this->diferenciaFecha(
              $_deuda->mat_fecha_vencimiento,
              date("Y-m-d")
            ) < 3
          ) {
            $this->TOTAL_os_desc_matricula =
              $this->TOTAL_os_desc_matricula + $_deuda->mat_monto;

            $_saldo_restante = $_saldo_restante - $_deuda->mat_monto;
          } else {
            // no esta correcto ,pero valido si con el interes puede descontarse
            if (
              $this->tieneSaldo(
                $_saldo_restante,
                $_deuda->mat_monto * $_deuda->mat_interes
              )
            ) {
              $this->TOTAL_os_desc_matricula =
                $this->TOTAL_os_desc_matricula +
              ...
              $_saldo_restante =
                $_saldo_restante - $_deuda->mat_monto * $_deuda->mat_interes;
            }
            ...
            if (strlen($this->INTERESES_pago_historico_id) === 0) {
              $this->INTERESES_pago_historico_id =
                $this->INTERESES_pago_historico_id . $_deuda->id_pago_historico;
            } else {
              $this->INTERESES_pago_historico_id =
                $this->INTERESES_pago_historico_id .
                "," .
                $_deuda->id_pago_historico;
            }
          }
        }
        if ($_deuda->id_concepto === 2) {
          $this->TOTAL_os_desc_fondo_sol =
            $this->TOTAL_os_desc_fondo_sol + $_deuda->mat_monto;

          $_saldo_restante = $_saldo_restante - $_deuda->mat_monto;
        }
        if (
          $_deuda->id_concepto !== 1 &&
          $_deuda->id_concepto !== 2 &&
          $_deuda->id_concepto !== 10 &&
          $_deuda->id_concepto !== 16 // AGREGADO POR QUE NO SE SABE QUE VA A SUCEDER CON ESA MATRICULA
        ) {
          $this->TOTAL_os_otros_ing_eg =
            $this->TOTAL_os_otros_ing_eg + $_deuda->mat_monto;
  ...
  private function actualizarDeudaLiquidacion(
    $id_pago_historico,
    ...
    $res = DB::update(
      DB::raw(
        "
  UPDATE mat_pago_historico SET mat_estado = 'P',  mat_fecha_pago = '" .
          $fecha .
          "', id_usuario = 1 , id_liquidacion_detalle = " .
          $id_liquidacion_detalle .
          " WHERE id_pago_historico IN  (" .
          $id_pago_historico .
          ") "
      )
    );
  ...
  {
    //ACTUALIZO LOS MONTO COBRADOS
    $fecha = date("Y-m-d");
    $res = DB::update(
      DB::raw(
        "
    UPDATE mat_pago_historico SET    mat_monto_cobrado = (mat_monto * " .
          $this->os_int_mora .
          ") WHERE id_pago_historico IN  (" .
          $_id_pago_historico .
          ") "
  ...
  private function actualizarLiquidacionDetalleConceptos(
    $id_liquidacion_detalle
  ) {
    $this->TOTAL_os_descuentos =
      $this->TOTAL_os_desc_matricula +
      $this->TOTAL_os_desc_fondo_sol +
      $this->TOTAL_os_otros_ing_eg; // valores cargados a mano para realizar correcciones , debitos  o creditos
    ...
    $res = DB::update(
      DB::raw(
        "
      UPDATE os_liq_liquidacion_detalle SET  os_ing_brutos = " .
          $this->TOTAL_os_ing_brutos .
          ",
      os_lote_hogar = " .
          $this->TOTAL_os_lote_hogar .
          ", os_gasto_admin = " .
  ...
  public function recalcularPagoHistorico(Request $request)
  {
    ...
    $res = DB::update(
      DB::raw(
        "
      UPDATE mat_pago_historico SET  mat_monto =  mat_monto_cobrado, mat_estado = 'A' , id_liquidacion_detalle = 0, id_liquidacion_generada = 0, mat_fecha_pago = '2099-12-31' WHERE id_liquidacion_generada IN  (" .
          $id_liquidacion_generada .
          ") "
      )
    );
  ...
  public function getLiquidacionDetalleByidLiquidacion(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw(
        "SELECT id_liquidacion_detalle, mat_matricula.mat_matricula_psicologo AS mat_matricula,CONCAT(mat_matricula.mat_apellido,' ', mat_matricula.mat_nombre) AS mat_apellidoynombre,
  ...
  AND os_liq_liquidacion_generada.id_liquidacion_generada = " .
          $id_liquidacion_generada .
          "
  "
      )
  ...
  WHERE  mat_matricula.mat_matricula_psicologo = os_liq_liquidacion_detalle.mat_matricula
  AND os_liq_liquidacion_detalle.id_liquidacion_generada = os_liq_liquidacion_generada.id_liquidacion_generada
  AND os_liq_liquidacion_detalle.mat_matricula = " .
          $mat_matricula .
  ...
  public function getOrdenByMatriculaAndLiquidacion(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw(
        "SELECT id_os_liq_orden, os_liq_orden.mat_matricula, os_liq_orden.id_obra_social, os_liq_orden.id_sesion, os_liq_orden.id_paciente, os_liq_orden.os_fecha,
  ...
  AND os_liq_liquidacion.id_liquidacion = " .
          $id_liquidacion .
          "
  AND os_liq_orden.mat_matricula = " .
          $mat_matricula .
          "
  "
      )
  ...
  public function getLiquidacionOrdenByExpediente(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw(
        "SELECT os_obra_social.id as id_obra_social, id_os_liq_orden, mat_matricula,CONCAT(mat_matricula.mat_apellido, ' ',mat_matricula.mat_nombre ) AS mat_apellido_nombre, os_liq_orden.id_sesion,
 ...
 AND  os_liq_liquidacion.id_os_liquidacion = " .
          $id_os_liquidacion .
          " ORDER BY id_os_liquidacion DESC
  "
      )
    );
    return response()->json($res, "200");
  ...
  public function getLiquidacionByMatriculaAndEstado(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT os_obra_social.id as id_obra_social, id_os_liq_orden, mat_matricula,CONCAT(mat_matricula.mat_apellido, ' ',mat_matricula.mat_nombre ) AS mat_apellido_nombre, os_liq_orden.id_sesion,
   ...
   AND mat_matricula.mat_matricula_psicologo = :mat_matricula AND os_estado_liquidacion = :estado ORDER BY id_os_liq_orden DESC
    "),
      ["mat_matricula" => $mat_matricula, "estado" => $estado]
    );
  ...
  public function getActuacionProfesionalByMatricula(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT os_liq_liquidacion.os_liq_numero, os_liq_liquidacion.os_fecha_desde,
  ...
  public function generarLiquidacionDetalle(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT os_liq_orden.mat_matricula, sum(os_precio_total) AS os_liq_bruto
  ...
  public function auditarOrdenes(Request $request)
  {
    ...
    while (isset($t[$i])) {
      $update = DB::table("os_liq_orden")
        ->where("id_os_liq_orden", $t[$i]["id_os_liq_orden"])
        ->limit(1)
        ->update([
  ...
  public function setDeuda(Request $request)
  {
    ...
    $tmp_fecha = str_replace(
      "/",
      "-",
      $request->input("mat_fecha_vencimiento")
    );
  ...
  public function afectarOrdenes(Request $request)
  {
    ...
    while (isset($request[$i])) {
      $update = DB::table("os_liq_orden")
        ->where("id_os_liq_orden", $request[$i]["id_os_liq_orden"])
        ->limit(1)
        ->update([
  ...
  public function getExpedienteByEstado(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT  `id_os_liquidacion`, `id_os_obra_social`, `os_liq_numero`, `os_fecha_desde`, `os_fecha_hasta`, `os_cant_ordenes`, `os_monto_total`, `os_estado`, `id_liquidacion`, os_nombre
  ...
  public function getExpedienteByIdLiquidacion(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT  `id_os_liquidacion`, `id_os_obra_social`, `os_liq_numero`, `os_fecha_desde`, `os_fecha_hasta`, `os_cant_ordenes`, `os_monto_total`, `os_estado`, os_liq_liquidacion_generada.id_liquidacion, os_liq_liquidacion_generada.id_liquidacion_generada, os_nombre
  ...
  public function desafectarExpediente(Request $request)
  {
    ...
    $estado = DB::update(
      DB::raw(
        "
         UPDATE os_liq_orden SET os_estado_liquidacion = 'AUD',  os_liq_numero = 0 WHERE os_estado_liquidacion = 'AFE' AND operacion_cobro.fecha_cobro  AND os_liq_numero= " .
          $os_liq_numero .
          "
      "
      )
    );
    DB::table("os_liq_liquidacion")
      ->where("id_os_liquidacion", "=", $os_liq_numero)
      ->delete();
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id_liquidacion_generada`, `id_liquidacion`, `os_fecha`, `os_liq_estado` FROM `os_liq_liquidacion_generada` ORDER BY  id_liquidacion_generada DESC
  ")
    );
  ...
  public function generarLiquidacion(Request $request)
  {
    ...
    while (isset($request[$i])) {
      ...
      $update = DB::table("os_liq_liquidacion")
        ->where("id_os_liquidacion", $request[$i]["id_os_liquidacion"])
        ->limit(1)
        ->update([
          "id_liquidacion" => $id_liquidacion,
        ]);
...
"
  ...
  {
    $res = DB::select(
      DB::raw("SELECT MAX(`os_num_ing_bruto`) AS os_num_ing_bruto FROM `os_liq_liquidacion_detalle` WHERE 1
    ")
    );
  ...
  public function getObrasSocialesByLiquidacion(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT DISTINCT os_obra_social.os_nombre , os_liq_liquidacion.os_liq_numero, os_liq_liquidacion.os_fecha_desde
  ...
  public function getUltimoNroIngBrutos(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT MAX(`os_num_ing_bruto`) as ultimo
    FROM `os_liq_liquidacion_detalle`
    ")
    );
  ...
  public function getUltimoNroRecibo(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT MAX(`num_comprobante`) as ultimo
    FROM `os_liq_liquidacion_detalle`
    ")
    );
  ...
  public function putActualizarNroIngBrutos(Request $request)
  {
    ...
    while (isset($request[$i])) {
      if ($request[$i]["os_liq_bruto"] >= 1500) {
        $estado = DB::update(
          DB::raw(
            "UPDATE os_liq_liquidacion_detalle
    SET os_num_ing_bruto = '" .
              $proximo_numero .
              "'
    WHERE id_liquidacion_detalle = '" .
              $request[$i]["id_liquidacion_detalle"] .
              "'
 ...
 "
  ...
  public function putActualizarValoresExpediente(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT * FROM os_liq_orden, os_sesion, os_liq_liquidacion WHERE os_liq_orden.id_sesion = os_sesion.id_sesion AND os_liq_orden.os_liq_numero = os_liq_liquidacion.id_os_liquidacion AND os_liq_liquidacion.id_os_liquidacion = :id_os_liquidacion
  "),
      [
  ...
  public function TESTobtenerDeudaMatricula(Request $request)
  {
    $mat_matricula = $request->input("mat_matricula");
    $res = DB::select(
      DB::raw(
