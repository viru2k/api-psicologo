...
class ObraSocialController extends ApiController
{
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `os_nombre`, `os_capitalizada`, `mat_obra_social`, `es_habilitada` FROM `os_obra_social`
      ")
    );
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `os_nombre`, `os_capitalizada`, `mat_obra_social`, `es_habilitada` FROM `os_obra_social` WHERE  es_habilitada = 'S'
      ")
    );
    return response()->json($res, "200");
  }

  public function getObraSocialPrivateHabilitado()
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `os_nombre`, `os_capitalizada`, `mat_obra_social`, `es_habilitada` FROM `os_obra_social`
      ")
  ...
  public function getConvenioByObraSocial(Request $request)
  {
    ...
    $res = DB::select(
      DB::raw("SELECT os_obra_social.id , os_nombre, es_habilitada, os_sesion.id_sesion, os_sesion.id_precio, os_sesion.os_sesion_mes, os_sesion.os_sesion_anual, os_sesion_tipo.os_sesion, os_sesion_tipo.os_sesion_codigo
  ...
  public function getConvenioPrivateByObraSocialHabilitado(Request $request)
  {
    $res = DB::select(
      DB::raw("SELECT os_obra_social.id , os_nombre, es_habilitada, os_sesion.id_sesion, os_sesion.id_precio, os_sesion.os_sesion_mes, os_sesion.os_sesion_anual, os_sesion_tipo.os_sesion, os_sesion_tipo.os_sesion_codigo
      FROM `os_obra_social`, os_sesion, os_sesion_tipo
  ...
  {
    $res = DB::select(
      DB::raw("SELECT os_obra_social.id , os_nombre, es_habilitada, os_sesion.id_sesion, os_sesion.id_precio,
  ...
  public function getConvenioPrivateHabilitado(Request $request)
  {
    $res = DB::select(
      DB::raw("SELECT os_obra_social.id , os_nombre, es_habilitada, os_sesion.id_sesion, os_sesion.id_precio,
        os_sesion.os_sesion_mes, os_sesion.os_sesion_anual, os_sesion_tipo.os_sesion, os_sesion_tipo.os_sesion_codigo, os_sesion.id_sesion_tipo, os_obra_social.id as obra_social_id
        FROM `os_obra_social`, os_sesion, os_sesion_tipo
        WHERE os_sesion.id_obra_social = os_obra_social.id AND os_sesion.id_sesion_tipo = os_sesion_tipo.id_sesion_tipo
         ORDER BY os_sesion.id_sesion DESC
        ")
    );
  ...
  {
    $res = DB::select(
      DB::raw("SELECT id_sesion_tipo, os_sesion, os_sesion_codigo FROM os_sesion_tipo
      ")
    );
